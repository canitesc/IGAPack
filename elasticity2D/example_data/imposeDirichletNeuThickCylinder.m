function [ stiff, rhs, bcdof, bcval ] = imposeDirichletNeuThickCylinder(stiff, rhs, PHTelem, GIFTmesh, p, q, bound_press)
%impose Dirichlet boundary conditions for thick cylinder
...
bcdof = [bcdof_down_y, bcdof_up_x];
bcval = [bcval_down_y, bcval_up_x];
...
for i_neu=1:size(neumann,1)
    ...
    for igauss = 1:ngauss_edge
        ...
        taux = -bound_press*normal(1)*ones(nument_edge,1);
        tauy = -bound_press*normal(2)*ones(nument_edge,1);
