function [quadRef,estErrorGlob]=recoverDerivEstGalExactErrMPPH(PHTelem, GIFTmesh, sol0, target_rel_error, quadList, p, q, Cmat, targetScale, rad, tx)
%recover by refining the elements with largest contribution to error, not relative contribution
%use the exact error rather than the estimated error
        ...
        for e=1:4 %for each element in the patch
            ...
            for jj=1:numGaussV
                for ii=1:numGaussU
                    ...
                    h1norm_recoverr = h1norm_recoverr + (stress_ex-recovStress)'*invC*(stress_ex-recovStress)*gaussWeightsU(ii)*gaussWeightsV(jj)*detJacobian*J;
                    h1norm_ex = h1norm_ex + stress_ex'*invC*stress_ex*gaussWeightsU(ii)*gaussWeightsV(jj)*detJacobian*J;
                end
            end
...
recoveredErrorGlob = sqrt(h1norm_recoverr/h1norm_ex)
%estError
