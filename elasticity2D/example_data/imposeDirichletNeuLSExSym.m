function [ stiff, rhs] = imposeDirichletNeuLSExSym(stiff, rhs, PHTelem, GIFTmesh, p, q)
    ...
    for i=1:length(PHTelem{patchIndex})
        if isempty(PHTelem{patchIndex}(i).children)
            if (patchIndex==2) && (isempty(PHTelem{patchIndex}(i).neighbor_left))
                if (PHTelem{patchIndex}(i).vertex(1)==0) && (PHTelem{patchIndex}(i).vertex(2)==0)
                    bcdof_reentrant = PHTelem{patchIndex}(i).nodesGlobal(1);                    
                end
            end
            ...
            if isempty(PHTelem{patchIndex}(i).neighbor_up)
                ...
                if (patchIndex==2) && (PHTelem{patchIndex}(i).vertex(3)==1) && (PHTelem{patchIndex}(i).vertex(4)==1)
                    bcdof_outer = PHTelem{patchIndex}(i).nodesGlobal(end);                    
                end
            end            
        end
    end
