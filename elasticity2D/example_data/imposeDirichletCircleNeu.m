function [ stiff, rhs, bcdof, bcval ] = imposeDirichletCircleNeu(stiff, rhs, PHTelem, GIFTmesh, p, q, bound_trac)
%impose Dirichlet boundary conditions for elastic circle
%fixed (homogeneous) boundary conditions on the left side of parameter
%space (one quarter circle)
%Neumann (traction) boundary conditions on the right side of parameter
%space (opposite quarter circle)
...
for i_neu=1:size(neumann,1)
    ...
    for igauss = 1:ngauss_edge
     ...
     %   dxdxi = dxdxi';
        ...
        taux = bound_trac(1)*nor(1)*ones(nument_edge,1);
        tauy = bound_trac(1)*nor(2)*ones(nument_edge,1);
