%script ElasticCUBEUV.m
% implement a cube domain with PHT splines
% use Multipatch with numUxnumV patches in each direction
% use GIFT mapping
...
numPatches = numPatchesU * numPatchesV;
...
GIFTmesh = init3DGeometryGIFTMP('cubeUV',L,W,H,numPatches,numPatchesU,numPatchesV);
...
while keep_refining
    ...
    [ stiff, rhs, bcdof, bcval ] = imposeDirichletECUV(stiff, rhs, PHTelem, p, q, r, bound_disp);
    ...
    toc
    vtuFile = ['ElasticCubeUVSol_p=',num2str(p),'_step',num2str(num_steps),'.vtu'];
