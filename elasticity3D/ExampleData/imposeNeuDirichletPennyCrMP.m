function [ stiff, rhs, bcdof, bcval ] = imposeNeuDirichletPennyCrMP(stiff, rhs, PHTelem, GIFTmesh, p, q, r, E, nu, a, sigma33)
...
for patchIndex = 1:length(PHTelem)
    for i=1:length(PHTelem{patchIndex})
        if isempty(PHTelem{patchIndex}(i).children)
            ...
            if (patchIndex==2) && (isempty(PHTelem{patchIndex}(i).neighbor_up))
                neumann_up = [neumann_up; i, 6, patchIndex];
            end
            ...
            if (patchIndex==2) && (isempty(PHTelem{patchIndex}(i).neighbor_down))
                if (PHTelem{patchIndex}(i).vertex(3)==0)
                    bcdof_downFace = [bcdof_downFace, PHTelem{patchIndex}(i).nodesGlobal(down_nodes)];
                end
            end
            
            if (patchIndex==1) && (isempty(PHTelem{patchIndex}(i).neighbor_front))
                if (PHTelem{patchIndex}(i).vertex(2)==0)
                    bcdof_frontFaces = [bcdof_frontFaces, PHTelem{patchIndex}(i).nodesGlobal(front_nodes)];
                end
            end
            
            if (patchIndex==2) && (isempty(PHTelem{patchIndex}(i).neighbor_front))
                if (PHTelem{patchIndex}(i).vertex(2)==0)
                    bcdof_frontFaces = [bcdof_frontFaces, PHTelem{patchIndex}(i).nodesGlobal(front_nodes)];
                end
            end
            
            if (patchIndex==1) && (isempty(PHTelem{patchIndex}(i).neighbor_back))
                if (PHTelem{patchIndex}(i).vertex(5)==1)
                    bcdof_backFaces = [bcdof_backFaces, PHTelem{patchIndex}(i).nodesGlobal(back_nodes)];
                end
            end
            
            if (patchIndex==2) && (isempty(PHTelem{patchIndex}(i).neighbor_back))
                if (PHTelem{patchIndex}(i).vertex(5)==1)
                    bcdof_backFaces = [bcdof_backFaces, PHTelem{patchIndex}(i).nodesGlobal(back_nodes)];
                end
            end
            ...
            if (patchIndex==1) && (isempty(PHTelem{patchIndex}(i).neighbor_left))
                if (PHTelem{patchIndex}(i).vertex(1)==0)
                    bcdof_leftFace = [bcdof_leftFace, PHTelem{patchIndex}(i).nodesGlobal(left_nodes)];
                end
            end
...
bcdof_downFace = unique(bcdof_downFace);
bcdof_frontFaces = unique(bcdof_frontFaces);
bcdof_backFaces = unique(bcdof_backFaces);
bcdof_leftFace = unique(bcdof_leftFace);

%take into account that there are 3 global indices (displacements) per node
bcdof_downFace_z = 3*bcdof_downFace;
bcdof_frontFaces_y = 3*bcdof_frontFaces-1;
bcdof_backFaces_x = 3*bcdof_backFaces-2;
bcdof_leftFace_x = 3*bcdof_leftFace-2;
bcdof_leftFace_y = 3*bcdof_leftFace-1;

%impose the prescribed displacement for each global index
bcval_downFace_z = zeros(size(bcdof_downFace_z));
bcval_frontFaces_y = zeros(size(bcdof_frontFaces_y));
bcval_backFaces_x = zeros(size(bcdof_backFaces_x));
bcval_leftFace_x = zeros(size(bcdof_leftFace_x));
bcval_leftFace_y = zeros(size(bcdof_leftFace_y));

bcdof = [bcdof_downFace_z, bcdof_frontFaces_y, bcdof_backFaces_x, bcdof_leftFace_x, bcdof_leftFace_y];
bcval = [bcval_downFace_z, bcval_frontFaces_y, bcval_backFaces_x, bcval_leftFace_x, bcval_leftFace_y];
...
%                                     hold on
            ...
            [~, ~, ~, stress] =  exact_penny(coord,E,nu,a,sigma33);
