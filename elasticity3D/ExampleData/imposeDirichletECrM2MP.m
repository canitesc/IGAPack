function [ stiff, rhs, bcdof, bcval ] = imposeDirichletECrM2MP(stiff, rhs, PHTelem, p, q, r, bound_disp)
...
for i=1:length(PHTelem{1})
    if isempty(PHTelem{1}(i).children)
        if isempty(PHTelem{1}(i).neighbor_left) && (PHTelem{1}(i).vertex(1)==0)
            bcdof_bottom = [bcdof_bottom, PHTelem{1}(i).nodesGlobal(left_nodes)];
        end
    end
end
...
for i=1:length(PHTelem{4})
    if isempty(PHTelem{4}(i).children)
        if isempty(PHTelem{4}(i).neighbor_left) && (PHTelem{4}(i).vertex(1)==0)
            bcdof_top = [bcdof_top, PHTelem{4}(i).nodesGlobal(left_nodes)];
        end
    end
end
...
bcval_bottom_x = -bound_disp*ones(size(bcdof_bottom_x));
bcval_bottom_y = zeros(size(bcdof_bottom_y));
bcval_bottom_z = zeros(size(bcdof_bottom_z));


bcval_top_x = bound_disp*ones(size(bcdof_top_x));
