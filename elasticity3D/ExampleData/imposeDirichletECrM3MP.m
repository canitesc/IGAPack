function [ stiff, rhs, bcdof, bcval ] = imposeDirichletECrM3MP(stiff, rhs, PHTelem, p, q, r, bound_disp)
...
bcval_bottom_y = -bound_disp*ones(size(bcdof_bottom_y));
bcval_bottom_z = zeros(size(bcdof_bottom_z));


bcval_top_x = zeros(size(bcdof_top_x));
bcval_top_y = bound_disp*ones(size(bcdof_top_y));
