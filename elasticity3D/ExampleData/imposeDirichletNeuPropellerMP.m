function [ stiff, rhs, bcdof, bcval ] = imposeDirichletNeuPropellerMP(stiff, rhs, PHTelem, GIFTmesh, p, q, r, bound_press, numBlades)
%impose Dirichlet and Neumann boundary conditions for the propeller
%fixed (homogeneous) boundary conditions on the back of the hub (front
%nodes)
%traction to the top of the blades pointing down
...
hubIndexSet = [];
for i=1:numBlades
    hubIndexSet = [hubIndexSet, 10*(i-1)+1:10*(i-1)+9];
end

bladeIndexSet = 10*(1:numBlades);

for indexPatch = hubIndexSet
    for i=1:length(PHTelem{indexPatch})
        if isempty(PHTelem{indexPatch}(i).children)
            if isempty(PHTelem{indexPatch}(i).neighbor_front)
                bcdof_fixed = [bcdof_fixed, PHTelem{indexPatch}(i).nodesGlobal(front_nodes)];
            end
        end
    end
end

for indexPatch = bladeIndexSet
    for i=1:length(PHTelem{indexPatch})
        if isempty(PHTelem{indexPatch}(i).children)
            if isempty(PHTelem{indexPatch}(i).neighbor_up)
                neumann_top = [neumann_top; i, 6, indexPatch];
            end
        end
    end
end
