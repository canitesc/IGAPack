function [ PHTelem, dimBasis ] = crossInsert3D( PHTelem, elm_index, dimBasis, p, q, r )
...
for e=elm_index
    ...
    [ bez, nodes, dimBasis ] = deCasteljau3dai( C_temp, xmin, xmax, ymin, ymax, zmin, zmax, p, q, r, [RTjunct, newBasisVert, 7], knotUl, knotUr, knotVd, knotVu, knotWb, knotWt, elmIn, dimBasis );
    ...
    for ichild = 1:8
        ...
        if (length(PHTelem(lastElem+ichild).neighbor_front)==1)
            PHTelem(PHTelem(lastElem+ichild).neighbor_front).neighbor_back = setdiff(unique([PHTelem(PHTelem(lastElem+ichild).neighbor_front).neighbor_back, lastElem+ichild]),e);
        end
        
        if (length(PHTelem(lastElem+ichild).neighbor_back)==1)
            PHTelem(PHTelem(lastElem+ichild).neighbor_back).neighbor_front = setdiff(unique([PHTelem(PHTelem(lastElem+ichild).neighbor_back).neighbor_front, lastElem+ichild]),e);
        end
        
        if (length(PHTelem(lastElem+ichild).neighbor_up_left)==1)
            PHTelem(PHTelem(lastElem+ichild).neighbor_up_left).neighbor_down_right = setdiff(unique([PHTelem(PHTelem(lastElem+ichild).neighbor_up_left).neighbor_down_right, lastElem+ichild]),e);
        end
        
        if (length(PHTelem(lastElem+ichild).neighbor_down_left)==1)
            PHTelem(PHTelem(lastElem+ichild).neighbor_down_left).neighbor_up_right = setdiff(unique([PHTelem(PHTelem(lastElem+ichild).neighbor_down_left).neighbor_up_right, lastElem+ichild]),e);
        end
        
        if (length(PHTelem(lastElem+ichild).neighbor_up_right)==1)
            PHTelem(PHTelem(lastElem+ichild).neighbor_up_right).neighbor_down_left = setdiff(unique([PHTelem(PHTelem(lastElem+ichild).neighbor_up_right).neighbor_down_left, lastElem+ichild]),e);
        end
        
        if (length(PHTelem(lastElem+ichild).neighbor_down_right)==1)
            PHTelem(PHTelem(lastElem+ichild).neighbor_down_right).neighbor_up_left = setdiff(unique([PHTelem(PHTelem(lastElem+ichild).neighbor_down_right).neighbor_up_left, lastElem+ichild]),e);
        end
        
        if (length(PHTelem(lastElem+ichild).neighbor_up_front)==1)
            PHTelem(PHTelem(lastElem+ichild).neighbor_up_front).neighbor_down_back = setdiff(unique([PHTelem(PHTelem(lastElem+ichild).neighbor_up_front).neighbor_down_back, lastElem+ichild]),e);
        end
        
        if (length(PHTelem(lastElem+ichild).neighbor_down_front)==1)
            PHTelem(PHTelem(lastElem+ichild).neighbor_down_front).neighbor_up_back = setdiff(unique([PHTelem(PHTelem(lastElem+ichild).neighbor_down_front).neighbor_up_back, lastElem+ichild]),e);
        end
        
        if (length(PHTelem(lastElem+ichild).neighbor_up_back)==1)
            PHTelem(PHTelem(lastElem+ichild).neighbor_up_back).neighbor_down_front = setdiff(unique([PHTelem(PHTelem(lastElem+ichild).neighbor_up_back).neighbor_down_front, lastElem+ichild]),e);
        end
        
        if (length(PHTelem(lastElem+ichild).neighbor_down_back)==1)
            PHTelem(PHTelem(lastElem+ichild).neighbor_down_back).neighbor_up_front = setdiff(unique([PHTelem(PHTelem(lastElem+ichild).neighbor_down_back).neighbor_up_front, lastElem+ichild]),e);
        end
        
        if (length(PHTelem(lastElem+ichild).neighbor_left_front)==1)
            PHTelem(PHTelem(lastElem+ichild).neighbor_left_front).neighbor_right_back = setdiff(unique([PHTelem(PHTelem(lastElem+ichild).neighbor_left_front).neighbor_right_back, lastElem+ichild]),e);
        end
        
        if (length(PHTelem(lastElem+ichild).neighbor_right_front)==1)
            PHTelem(PHTelem(lastElem+ichild).neighbor_right_front).neighbor_left_back = setdiff(unique([PHTelem(PHTelem(lastElem+ichild).neighbor_right_front).neighbor_left_back, lastElem+ichild]),e);
        end
