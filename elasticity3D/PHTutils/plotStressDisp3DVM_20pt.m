function plotStressDisp3DVM_20pt(PHTelem, GIFTmesh, sol0, p, q, r, Cmat, vtuFile, edgeSpace)
    ...
    for i=1:length(PHTelem{patchIndex})
        if isempty(PHTelem{patchIndex}(i).children)
            ...
            for listIndex = 1:size(list_ijk,1)
                ...
                sigmaVM(1,gp,e) = sqrt(1/2*((sigma(1,gp,e)-sigma(2,gp,e))^2+(sigma(2,gp,e)-sigma(3,gp,e))^2+(sigma(3,gp,e)-sigma(1,gp,e))^2+6*(sigma(4,gp,e)^2+sigma(5,gp,e)^2+sigma(6,gp,e)^2)));
                gp = gp +1 ;
...
msh_to_vtu_3dVM_20points(x, u, sigma, sigmaVM, [numPlotX numPlotY numPlotZ], vtuFile);