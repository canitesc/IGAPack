function [octupleRef,estErrorGlob, estError]=recoverDerivEstGalMPAll(PHTelem, GIFTmesh, sol0, target_rel_error, octupleList, p, q, r, Cmat, targetScale)
...
local_target = targetScale/numOctuples*target_rel_error^2*h1norm_glob;
for indexPatch = 1:numPatches
    for j=1:size(octupleList{indexPatch},1)
        sumError = sum(estError{indexPatch}(octupleList{indexPatch}(j,:)));
        if sumError > local_target
            %mark the patch for refinement
            octupleRef{indexPatch}(1,j) = sumError;
        end
    end
end