function [octupleRef,estErrorGlob, estError]=recoverDerivEstGal(PHTelem, GIFTmesh, sol0, target_rel_error, octupleList, p, q, r, Cmat)
    ...
    for kk=1:length(uConvPts_w)
        for jj=1:length(uConvPts_v)
            for ii=1:length(uConvPts_u)
                ...
                scrtx = PHTelem(e_glob).nodes(1:nument);
        ...
        for kk=1:numGaussW
            for jj=1:numGaussV
                for ii=1:numGaussU
                    ...
                    h1norm_glob = h1norm_glob + recovStress'*invC*recovStress*gaussWeightsU(ii)*gaussWeightsV(jj)*gaussWeightsW(kk)*detJacobian;
                    h1normerr = h1normerr + (recovStress-compStress)'*invC*(recovStress-compStress)*gaussWeightsU(ii)*gaussWeightsV(jj)*gaussWeightsW(kk)*detJacobian;
...
local_target = 1/size(octupleList,1)*target_rel_error^2*h1norm_glob;
for j=1:size(octupleList,1)
