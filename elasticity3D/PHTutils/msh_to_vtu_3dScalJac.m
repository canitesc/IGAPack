function msh_to_vtu_3dScalJac(pts, u, stress, scaledJac, npts, filename)

% MSH_TO_VTU_3DVM: Export an unstructured mesh to VTK format for plotting.
%
%  msh_to_vtu_3dVM (pts, u, stress, scaledJac, npts, filename, fieldname)
%
% INPUT:
%
%     pts:       points at which the field was computed
%     u:         values of the displacement field at the selected points
%     stress:    values of the stresses at the selected points
%     scaledJac:  values of the von Misses stresses at the selected points
...
% Modified by www.itn-insist.com
...
str7 = cat(2, '</DataArray> \n', ...
    '<DataArray  type="Float64"  Name="scaledJac" NumberOfComponents="1" format="ascii"> \n');
...
if (ndim == 2)
    ...
    %local_conn = [0 8 1 11 9 3 10 2 16 17 19 18 4 12 5 15 13 7 14 6];
    
    local_conn = [0 2 7 5 12 14 19 17 1 4 6 3 13 16 18 15 8 9 11 10];
end

fprintf (fid, str1, npoints, ncells, fieldclass, fieldclass, 'u', ncomp);
fprintf (fid, '%g ', u(:));
fprintf (fid, '</DataArray> \n');
fprintf (fid, '<DataArray  type="Float64"  Name="stress" NumberOfComponents="6" format="ascii"> \n');
fprintf (fid, '%g  ', stress(:));

fprintf (fid, str7);
fprintf (fid, '%g  ', scaledJac(:));

fprintf (fid, str2);
fprintf (fid, '%g ', pts(:));
fprintf (fid, str3);
for iel = 1:nel
    fprintf (fid, '%d ', local_conn + (iel-1)*20);
end
fprintf (fid, str4);
%fprintf (fid, '%d ', 8:8:ncells*8);
fprintf (fid, '%d ', 20:20:ncells*20);
...
if (ndim == 2)
   ...
   fprintf (fid, '%d ', 25 * ones (1, ncells));
end
fprintf (fid, str6);
